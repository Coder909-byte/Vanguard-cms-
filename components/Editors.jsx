...
export default function Editor({ onSave, initialData }) {
  ...
  const handleForm = (data) => {
    try {
      ...
      const slugifiedCategory = slugify(data.category);
      const payload = {
        ...data,
        slug: generatedSlug,
  ...
  const handleGenerateContentUsingAI = async (e) => {
    e.preventDefault();
    try {
  ...
  return (
    <section>
      <form className="space-y-4" onSubmit={handleSubmit(async(data)=> {
        if(status === 'loading'){
          toast({ title: "Error", description: "Please wait, session is loading", variant: "destructive"})
          return;
        }
        if(!session?.user?.id){
